...
constexpr std::size_t kMaxStoredSnapshots = 7300;
...
thread_local bool g_snapshot_bulk_active = false;
...
bool parse_native_render_speed(const char *text, std::uint32_t *speed_quarters_out) {
  ...
  constexpr SpeedChoice kChoices[] = {
      {"0.25", 1}, {"0.5", 2}, {"1", 4}, {"1.0", 4}, {"2", 8}, {"2.0", 8}, {"4", 16}, {"4.0", 16}, {"8", 32}, {"8.0", 32}, {"16", 64}, {"16.0", 64},
  };
...
bool process_range_is_readable(const void *address, std::size_t size) {
  ...
  // Android ARM64 heap pointers may carry a top-byte tag; /proc/self/maps
  // contains untagged addresses. Normalize only the range-check address.
  const std::uintptr_t begin = reinterpret_cast<std::uintptr_t>(address) & 0x00ffffffffffffffULL;
  if (begin > UINTPTR_MAX - size) {
    return false;
  }
  const std::uintptr_t end = begin + size;
...
void complete_snapshot_operation_locked(std::uint64_t sequence, bool succeeded, SnapshotError error,
  ...
  if (g_snapshot_bulk_active) return;
...
void process_snapshot_operation_on_worker(void *manager, RunnerMode mode, std::uint64_t sequence,
  ...
  if (operation == SnapshotOperation::Clear) {
    // Runs on the owning thread, just like snapshot serialization/restoration.
    for (auto &snapshot : g_stored_snapshots) {
      if (snapshot.wrapper && g_state_snapshot_delete) g_state_snapshot_delete(snapshot.wrapper);
      snapshot = {};
    }
    pthread_mutex_lock(&g_control_mutex);
    complete_snapshot_operation_locked(sequence, true, SnapshotError::None);
    pthread_mutex_unlock(&g_control_mutex);
    return;
  }
  if (operation == SnapshotOperation::Adopt) {
    pthread_mutex_lock(&g_control_mutex);
    SnapshotManagerIdentity target;
    bool valid = mode == RunnerMode::NativeRender && g_control_ended &&
        current_snapshot_identity_locked(mode, &target) && g_game_state_tick(manager) == 90 &&
        g_active_replay_json[0] && std::strcmp(g_active_replay_json, g_pending_native_render_json) == 0;
    std::size_t count = 0;
    for (const auto &saved : g_stored_snapshots) {
      if (!saved.wrapper) continue;
      ++count;
      valid = valid && saved.mode == RunnerMode::Headless && saved.generation == g_control_generation &&
              saved.config_revision == g_config_processed_sequence;
    }
    if (valid && count) {
      for (auto &saved : g_stored_snapshots) {
        if (!saved.wrapper) continue;
        saved.mode = mode; saved.generation = target.generation;
        saved.config_revision = target.config_revision; saved.state_epoch = target.state_epoch;
      }
    }
    complete_snapshot_operation_locked(sequence, valid && count,
        valid && count ? SnapshotError::None : SnapshotError::StaleHandle);
    pthread_mutex_unlock(&g_control_mutex);
    return;
  }
  if (operation == SnapshotOperation::Precompute) {
    // Headless worker only: no controller, scene, view events or frame clock.
    g_snapshot_bulk_active = true;
    for (std::uint64_t i = 0; i < requested_handle; ++i) {
      BattleResultView result;
      if (read_battle_result(read_object_field<void *>(manager, 0xa8), &result) && result.finalized) break;
      const auto before = g_game_state_tick(manager);
      process_scheduled_replay_actions(manager);
      g_original_game_state_step(manager);
      const auto tick = g_game_state_tick(manager);
      pthread_mutex_lock(&g_control_mutex);
      ++g_completed_steps;g_last_controlled_tick = tick;
      if (read_battle_result(read_object_field<void *>(manager, 0xa8), &result) && result.finalized)
        g_control_ended = true;
      pthread_mutex_unlock(&g_control_mutex);
      if (tick != before + 1) {
        g_last_snapshot_operation_succeeded = false;
        g_last_snapshot_operation_error = SnapshotError::CaptureFailed;
        break;
      }
      process_snapshot_operation_on_worker(manager, mode, sequence, SnapshotOperation::Create, 0);
      if (!g_last_snapshot_operation_succeeded) break;
    }
    g_snapshot_bulk_active = false;
    pthread_mutex_lock(&g_control_mutex);
    complete_snapshot_operation_locked(sequence, g_last_snapshot_operation_succeeded,
                                       g_last_snapshot_operation_error, g_last_snapshot_operation_result);
    pthread_mutex_unlock(&g_control_mutex);
    return;
  }
  if (operation == SnapshotOperation::Create) {
    ...
    for (const auto &saved : g_stored_snapshots) cache_bytes += saved.serialized_bytes;
    slot = cache_bytes < 256ULL * 1024 * 1024 ? find_free_snapshot_slot_locked() : nullptr;
    if (slot == nullptr) {
  ...
  if (operation == SnapshotOperation::Restore) {
    ...
    if (valid_prelude) {
      if (mode == RunnerMode::NativeRender) {
        // Stock restore also notifies the view listeners (cards, arena state,
        // end-of-battle presentation). Raw logic deserialization omits these.
        void *controller = g_native_render_controller.load(std::memory_order_acquire);
        void *sync = controller ? read_object_field<void *>(controller, 0x80) : nullptr;
        const std::uint8_t prologue[16] = {0xff,0x03,0x02,0xd1,0xfd,0x7b,0x03,0xa9,
                                          0xfa,0x67,0x04,0xa9,0xf8,0x5f,0x05,0xa9};
        auto *restore_entry = reinterpret_cast<void *>(g_libg_base + 0xccebc8);
        if (sync && read_object_field<void *>(sync, 0x08) == manager &&
            std::memcmp(restore_entry, prologue, sizeof(prologue)) == 0) {
          g_byte_stream_reset_read(stream);
          void *vtable = read_object_field<void *>(stream, 0);
          auto read_byte = reinterpret_cast<ByteStreamReadValue>(read_object_field<void *>(vtable, 0x150));
          read_byte(stream); read_byte(stream); // stock entry consumes has-full-state itself
          reinterpret_cast<bool (*)(void *, void *)>(restore_entry)(sync, stream);
        }
      } else {
...
SnapshotOperationReceipt request_snapshot_operation(SnapshotOperation operation, std::uint64_t handle) {
  ...
  const bool restore_native = (operation == SnapshotOperation::Restore || operation == SnapshotOperation::Precompute || operation == SnapshotOperation::Clear || operation == SnapshotOperation::Adopt) && mode == RunnerMode::NativeRender;
...
// Declared before the resident batch plane, defined after minimal_capture.inc.
// Lets the batch data plane ship the compact minimal state (~1 KiB) instead of
// the multi-KiB full atomic observation: the full CRTF training capture depends
// on the runtime telemetry hooks, which the minimal build does not install.
bool build_minimal_batch_capture(char *buffer, std::size_t capacity, std::size_t *size_out);
#endif
#include "resident_multimatch.inc"
#include "engine_instance.inc"
#include "minimal_capture.inc"
#ifdef CR_MINIMAL_CAPTURE
bool build_minimal_batch_capture(char *buffer, std::size_t capacity, std::size_t *size_out) {
  pthread_mutex_lock(&g_control_mutex);
  // cursor=0 means "from this bind's epoch start". resident_bind_slot resets the
  // combat event epoch on every slot switch, so this is exactly the plays that
  // happened during the current step; a cross-bind cursor would go stale.
  const bool ok = build_minimal_capture(buffer, capacity, 0, 0);
  pthread_mutex_unlock(&g_control_mutex);
  if (ok && size_out != nullptr) {
    *size_out = std::strlen(buffer);
  }
  return ok;
}
#endif
...
void handle_control_command(int socket_fd, const char *command) {
  char response[kControlResponseBytes] = {};
  if (std::strcmp(command, "offline on") == 0 || std::strcmp(command, "offline off") == 0) {
    const bool active = std::strcmp(command, "offline on") == 0;
    g_offline_control_session_active.store(active, std::memory_order_release);
    send_control_response(socket_fd, active ? "{\"ok\":true,\"offline\":true,\"replayStudioVersion\":5}" : "{\"ok\":true,\"offline\":false}");
    return;
  }
#ifdef CR_MINIMAL_CAPTURE
  if (handle_minimal_capture(socket_fd, command)) return;
  // Unsupported rich/snapshot commands must not imply full telemetry support.
  // CR_RESIDENT_COMMANDS additionally exposes the resident multi-match control
  // plane (env-prefixed commands, multi-status, multi-stop) so several battles
  // can be driven in one host round trip. Every baseline minimal command keeps
  // working unchanged, so this build is a strict superset of the baseline.
  bool resident_allowed = false;
#ifdef CR_RESIDENT_COMMANDS
  resident_allowed = std::strncmp(command, "env ", 4) == 0 || std::strcmp(command, "multi-status") == 0 ||
                     std::strcmp(command, "multi-stop") == 0 || std::strcmp(command, "engine-status") == 0;
#endif
  const bool native_render_allowed = std::strcmp(command, "cache-adopt") == 0 ||
                                     std::strncmp(command, "cache-step ", 11) == 0 ||
                                     std::strcmp(command, "cache-clear") == 0 ||
                                     std::strcmp(command, "snapshot-create") == 0 ||
                                     std::strncmp(command, "restore ", 8) == 0 ||
                                     std::strncmp(command, "release-snapshot ", 17) == 0 ||
                                     std::strncmp(command, "configure-native ", 17) == 0 ||
                                     is_replay_schedule_command(command) ||
                                     std::strncmp(command, "play ", 5) == 0 ||
                                     std::strncmp(command, "replay-schedule-card ", 21) == 0 ||
                                     std::strncmp(command, "advance-native ", 15) == 0 ||
                                     std::strncmp(command, "speed ", 6) == 0 ||
                                     std::strcmp(command, "pause") == 0 || std::strcmp(command, "resume") == 0;
  ...
  if (g_resident_mode_active &&
      (std::strncmp(command, "configure ", 10) == 0 || std::strncmp(command, "configure-native ", 17) == 0 ||
  ...
  if (std::strcmp(command, "cache-adopt") == 0) {
    const auto receipt = request_snapshot_operation(SnapshotOperation::Adopt, 0);
    send_control_response(socket_fd, receipt.completed && receipt.succeeded ?
      "{\"ok\":true}" : "{\"ok\":false,\"error\":\"headless cache does not match rendered match\"}");
    return;
  }
  if (std::strcmp(command, "cache-clear") == 0 || std::strncmp(command, "cache-step ", 11) == 0) {
    const bool clear = std::strcmp(command, "cache-clear") == 0;
    std::uint64_t ticks = 0;
    if (!clear && (!parse_positive_uint64(command + 11, &ticks) || ticks > 512)) {
      send_control_error(socket_fd, "cache-step expects 1..512 ticks"); return;
    }
    pthread_mutex_lock(&g_control_mutex);
    const auto first_handle = g_next_snapshot_handle;
    const bool headless = g_runner_mode.load() == static_cast<int>(RunnerMode::Headless) &&
        !g_resident_mode_active && !g_controlled_step_active && g_step_budget == 0;
    pthread_mutex_unlock(&g_control_mutex);
    if (!clear && !headless) {
      send_control_error(socket_fd, "precomputation requires idle legacy headless engine"); return;
    }
    const auto receipt = request_snapshot_operation(clear ? SnapshotOperation::Clear : SnapshotOperation::Precompute, ticks);
    std::string payload = "{\"ok\":";
    payload += receipt.completed && receipt.succeeded ? "true" : "false";
    payload += ",\"error\":\"";
    payload += snapshot_error_message(receipt.error);
    payload += "\",\"snapshots\":[";
    pthread_mutex_lock(&g_control_mutex);
    bool first = true;
    for (const auto &saved : g_stored_snapshots) {
      if (!saved.wrapper || saved.handle > receipt.result.handle ||
          saved.handle < first_handle || clear) continue;
      char entry[192];
      std::snprintf(entry, sizeof(entry), "%s{\"tick\":%d,\"handle\":%llu,\"bytes\":%u,\"digest\":\"%016llx\"}",
                    first ? "" : ",", saved.tick, static_cast<unsigned long long>(saved.handle),
                    saved.serialized_bytes, static_cast<unsigned long long>(saved.digest));
      payload += entry; first = false;
    }
    pthread_mutex_unlock(&g_control_mutex);
    payload += "]}";
    send_control_response(socket_fd, payload.c_str()); return;
  }
  ...
  if (std::strncmp(command, "speed ", 6) == 0) {
    ...
    if (!parse_native_render_speed(command + 6, &speed_quarters)) {
      send_control_error(socket_fd, "speed must be one of 0.25, 0.5, 1, 2, 4, 8, 16");
      return;
    }
    if (g_runner_mode.load(std::memory_order_acquire) != static_cast<std::int32_t>(RunnerMode::NativeRender)) {
  ...
  send_control_error(
      ...
      "replay-schedule-clear, inject JSON, speed 0.25|0.5|1|2|4|8|16, pause, resume");
}
...
void game_state_step_hook(void *manager) {
  ...
  for (;;) {
    ...
    while (g_controlled_manager == manager && g_control_gate_enabled && g_step_budget == 0 &&
           ...
           (g_snapshot_request_sequence == g_snapshot_processed_sequence ||
            g_pending_snapshot_mode != RunnerMode::Headless || g_pending_snapshot_manager != manager) &&
  ...
  if (native_render_manager && g_native_render_manager.load(std::memory_order_acquire) == manager &&
    ...
    // Natural playback must stop on the same terminal tick as cached stepping.
    // Further callbacks in a high-speed frame are rejected by the pause gate.
...
#ifdef CR_MINIMAL_CAPTURE
  success &= install_minimal_capture_hook(libg_base);
#else
