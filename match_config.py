...
def make_match(decks, *, level=16, forms=None, towers=(159000000, 159000000),
    ...
    forms = ((0,) * 8, (0,) * 8) if forms is None else tuple(tuple(row) for row in forms)
    if len(forms) != 2 or any(len(row) != 8 or any(type(v) is not int or v not in (0, 1, 2, 3) for v in row) for row in forms):
        raise ValueError('forms must contain two eight-element masks (0=base, 1=evolution, 2=hero, 3=both)')
    ...
    for owner in range(2):
        battle[f'deck{owner}']['sp'] = [dict(d=card, el=mask) for card, mask in zip(decks[owner], forms[owner])]
