...
class DeviceSession:
    ...
    def rules(self, active):
        ...
        uid = self.shell(
            "stat", "-c", "%u", "/data/user/0/" + boot.PACKAGE
        ).stdout.strip()
        if not uid.isdigit():
        ...
        for table in ("iptables", "ip6tables"):
            ...
            nums = [
                ...
                if boot.RULE_COMMENT in line and line.split()[0].isdigit()
            ]
    ...
    def start_game(self):
        try:
            self.shell("am", "start", "-n", boot.ACTIVITY)
    ...
    def prepare_engine(self):
        ...
        self.ensure_root()
        directory = boot.app_directory(self.cfg["adb"], self.cfg["serial"])
        libdir = directory / "lib/arm64"
        manifest = json.loads(
            (ROOT / "vendor/firstlight/supported_engine.json").read_text()
        )
        ...
        probe = ROOT / "build/libcrprobe.so"
        if not probe.exists():
            raise RuntimeError("缺少 build/libcrprobe.so，请先构建本地探针")
        ...
        for attempt in range(3):
            ...
            pid = self.shell("pidof", boot.PACKAGE).stdout.strip().split()[0]
            try:
    ...
    """.replace(
        ...
        self.script.load()
        boot.wait_until_ready(self.cfg["port"], 90)
        if errors:
    ...
    def dismiss_connection_dialog(self):
        ...
        dialog = any(
            boot.ACTIVITY in w.splitlines()[0]
            and "package=" + boot.PACKAGE in w
            and "(wrapxwrap)" in w
