...
def extract(path):
    ...
    for item in parser.timeline:
        ...
        events.append({
            ...
            'source_x': None if marker.get('data-x') in (None, 'None') else int(marker['data-x']),
            'source_y': None if marker.get('data-y') in (None, 'None') else int(marker['data-y']),
            'source_marker_index': int(marker['data-i']) if marker.get('data-i') is not None else None,
        })
    ...
    return {
        ...
        'unresolved': ['native time/coordinate mapping', 'initial hand and random seed',
                       'card levels, forms and rules compatibility',
                       'ability identity and parameters', 'exact terminal tick'],
        'counts': {side: {kind: counts[side, kind] for kind in ('card_play', 'ability')}
...
if __name__ == '__main__':
    ...
    print(json.dumps({'events': len(data['events']), 'counts': data['counts']}, ensure_ascii=False))
